 ...
 * 取出指定分类的图片数据
 */
function getCatePic($cat_id,$size=null){
  if(is_null($size)) $size=IMG_THUMB;

  $sql="select image_id from ".IMAGE_CATEGORY_TABLE." where category_id={$cat_id} order by rank desc";
  $temp=query2array($sql);
  $ids_array=array();
  foreach($temp as $v){
    $ids_array[]=$v['image_id'];
  }

  $query = 'SELECT * FROM '.IMAGES_TABLE.' WHERE id IN ('.implode(',', $ids_array).') order by find_in_set(id,"'.implode(',',$ids_array).'") ;';
  ...
  foreach ($data as $row){
    $url = duplicate_picture_url(array('image_id' => $row['id'],'category'=>$cate));
  ...
  global $template;
  $template->assign('derivative_params'.$cat_id,ImageStdParams::get_by_type( pwg_get_session_var('index_deriv',$size) ));
  // $template->assign(array(
  //   'derivative_params' => trigger_change('get_index_derivative_params', ImageStdParams::get_by_type( pwg_get_session_var('index_deriv',$size) ) ),
  // ));
  return $cateData;
}
/**
 * 取出指定父分类下的所有图片数据
 */
function getPidCatePic($cat_id,$size=null){
  
  $sql="select id from ".CATEGORIES_TABLE." where id_uppercat={$cat_id}";
  $temp=query2array($sql);
  $cids_array=array();
  foreach($temp as $v){
    $cids_array[]=$v['id'];
  ...
  if(is_null($size)) $size=IMG_THUMB;
  $sql="select a.image_id,b.name,b.id from ".IMAGE_CATEGORY_TABLE." as a left join ".
    CATEGORIES_TABLE." as b".
    " on a.category_id=b.id".
    " where a.category_id in(".implode(',', $cids_array).") order by a.rank desc";

 
  $temp=query2array($sql);
  $ids_array=array();
  foreach($temp as $v){
    $cat[$v['image_id']]=array('category_id'=>$v['id'],'category_name'=>$v['name']);
    $ids_array[]=$v['image_id'];
  }

  $query = 'SELECT * FROM '.IMAGES_TABLE.' WHERE id IN ('.implode(',', $ids_array).') order by id desc;';
  $data=query2array($query);
  ...
  foreach ($data as $k=>$row){
      $cate=array(
        'name'=>'',
        'id'=>$cat[$row['id']]['category_id'],
        'permalink'=>$cat[$row['id']]['category_id']
      );
    $data[$k]['category']=$cat[$row['id']];
    $url = duplicate_picture_url(array('image_id' => $row['id'],'category'=>$cate));
    $tpl_var = array_merge($row, array(
      'URL' => $url,
      'src_image' => new SrcImage($row),
