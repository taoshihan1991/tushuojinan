...
/*详情页左右箭头url*/
// $nextUrl = duplicate_picture_url(array(
//     'image_id' => $picture['next']['id'],
//     'start' => $page['start'],
//   )
// );
// $picture['next']['url']=$nextUrl;
// $previousUrl = duplicate_picture_url(array(
//     'image_id' => $picture['previous']['id'],
//     'start' => $page['start'],
//   )
// );
// $picture['previous']['url']=$previousUrl;
...
if(!empty($page['items'])){
  ...
  foreach($page['items'] as $key=>$v){
    if($v==$page['image_id']){
      $page['start']=ceil(($key+1)/12)-1;
      $page['start']= $page['start']<=0 ? 0 : $page['start'];
    }
  }

  $pictures = array();
  $selection = array_slice($page['items'],$page['start']*12,12);
  ...
  foreach ($pictures as $row){
    $url = duplicate_picture_url(array('image_id' => $row['id'],'image_file' => $row['file']),
      array('start'));
  ...
  if($index){
      ...
      $prevKey=$prev*12;
      $prevUrl = duplicate_picture_url(array('image_id' => $page['items'][$prevKey],'',array('start')));
      $template->assign('prevUrl',$prevUrl);

      $next=$index+1;
      $nextKey=$next*12;
      $nextUrl = duplicate_picture_url(array('image_id' => $page['items'][$nextKey],'',array('start')));
      $template->assign('nextUrl',$nextUrl);
  }
